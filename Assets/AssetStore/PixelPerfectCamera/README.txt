...
3) You can specify either the target width or height in pixels. This is not screen pixels, but pixels of your sprites (asset pixels).
...
PixelSnap script can produce a more "snappy" old-school movement of your game object (`Retro Snap`) or help you alleviate sub-pixel positioning artifacts such as jittering (`Pixel Snap`).

1) Place the script on every object that you want to enable pixel snapping on.

2) Choose the `Retro Snap` or the `Pixel Snap` option in your PixelPerfectCamera's `Pixel Snap mode` setting. **All** the objects that use the PixelSnap script will render in the chosen mode.

3) PixelSnap has a `Pixel Snap Choice mode` option that allows you to disable pixel snapping on a per object basis (if you set it to `Off`). If you set it to `Force Pixel Snap`,  PixelSnap will be applied on the object regardles of the PixelPerfectCamera setting. `Let camera choose` is the default setting, which works as explained earlier.

"Retro Snap" should be used with "Pixel Perfect" enabled, otherwise jitter artifacts will occur.
